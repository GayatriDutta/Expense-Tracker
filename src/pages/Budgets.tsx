...
import { Plus, CreditCard as Edit3, Trash2, Target, AlertTriangle, CheckCircle, ArrowLeft } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../contexts/AuthContext';
import { formatCurrency } from '../utils';
import { createBudget, deleteBudget, getAllBudget, getAllCategories, getAllExpenses, updateBudget } from '../api/service.api';
...
const Budgets: React.FC = () => {
  ...
  const getBudgetStatus = (budget: Budget) => {
    const monthExpenses = expenses.filter(expense => {
      ...
      const matchesCategory = budget.category.name != 'All Categories' ? expense.categoryId === budget.categoryId : true;
              ...
              >
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between text-sm text-gray-600 dark:text-gray-400">
                    <span>Remaining: {formatCurrency(Math.max(0, Number(budget.amount) - spent))}</span>
                    {percentage > 100 && (
